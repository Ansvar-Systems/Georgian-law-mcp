/**
 * Tool registry for Georgian Law MCP Server.
...
import { getGeorgianImplementations, type GetGeorgianImplementationsInput } from './get-georgian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Legislative Herald of Georgia. ' +
    ...
    'Call this FIRST when you need to understand what Georgian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Georgian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Georgian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Georgian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Georgian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Georgian?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Georgian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Georgian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that a Georgian statute references or aligns with. ' +
      'Georgian is an EU candidate state aligning its legislation with the EU acquis ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Georgian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_georgian_implementations',
    description:
      'Find all Georgian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Georgian statutes. ' +
      'Georgian transposes EU directives into national law as an EU candidate state.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Georgian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_georgian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Georgian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Georgian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Georgian statute or provision. ' +
      ...
      'Georgian is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Georgian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_georgian_implementations':
          result = await getGeorgianImplementations(db, args as unknown as GetGeorgianImplementationsInput);
          break;
