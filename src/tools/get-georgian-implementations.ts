/**
 * get_georgian_implementations — Find Georgian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetGeorgianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface GeorgianImplementationResult {
  document_id: string;
...
export async function getGeorgianImplementations(
  db: InstanceType<typeof Database>,
  input: GetGeorgianImplementationsInput,
): Promise<ToolResponse<GeorgianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as GeorgianImplementationResult[];
