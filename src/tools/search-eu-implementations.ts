/**
 * search_eu_implementations — Search EU directives/regulations referenced by Georgian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_georgian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as georgian_statute_count
  ...
  if (input.has_georgian_implementation) {
    sql += ' HAVING georgian_statute_count > 0';
  }
