#!/usr/bin/env tsx
/**
 * Georgian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://matsne.gov.ge';
const PORTAL_NAME = 'Matsne (Legislative Herald of Georgia)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/georgian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Georgian Law MCP — Data Freshness Check');
